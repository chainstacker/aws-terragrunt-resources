...
dependency "sg" {
  config_path                             = "../../security_groups/code_auditor_frontend"
  mock_outputs_allowed_terraform_commands = ["init", "validate", "plan"]
  mock_outputs = {
    security_group_id = "sg-02092eea015a2eade"
  }
...
inputs = {
  ...
  name             = "${local.locals.project_name}-frontend-app"
  cpu              = 1024
  memory           = 2048
  ...
  container_definitions = {
    test-app-frontend = {
      ...
      secrets = [
        {
          "name" : "BACKEND_URL",
          "valueFrom" : "arn:aws:secretsmanager:eu-central-1:654654236251:secret:TEST_APP_FRONTEND_SECRET-cVlZLe::"
        }
      ]
      ...
      log_configuration = {
        logDriver = "awslogs"
        options = {
          awslogs-group         = "test-app-frontend"
  ...
  task_exec_iam_role_name     = "${local.locals.project_name}-frontend-app-ecs-role"
